...
export default function Home() {
  ...
  const [cookies] = useCookies(["selectedCalendars"])

  const [freeTimeSlots, setFreeTimeSlots] = useState([])
  useEffect(() => {
    const getEvents = async () => {
      const nomarlisedEvents = await calendar.getEvents(cookies.selectedCalendars)
      const freeTimeSlots = calendar.findTimeSlot(
        nomarlisedEvents,
        highlightTime.valueGroups,
        moment().set("hour", 22).set("minute", 0)
      )
      setFreeTimeSlots(freeTimeSlots)
    }
    getEvents()
  }, [highlightTime, cookies.selectedCalendars])
                ...
                render={({ next }) => (
                  <Box className={classes.card}>
                    ...
                    <Typography variant="body1">
                      Write down what you really want to get done today. This
                      will be your focus.
                    </Typography>
                    <TextField
                      variant="filled"
              ...
              <Step
                ...
                render={() => (
                  <Box className={classes.card}>
                    ...
                    <Typography>
                      You have {freeTimeSlots.length} time slots to focus on
                      your highlight. Which one do you want to schedule?
                    </Typography>
                    <Box className={classes.evenlySpacedCentered}>
              ...
              <Step
                ...
                render={() => (
                  <Box className={classes.card}>
                    ...
                    <Typography>
                      You don't have any time slots available at the moment.
                      What do you want to do to plan in your highlight?
                    </Typography>
                  </Box>
                )}
              />
